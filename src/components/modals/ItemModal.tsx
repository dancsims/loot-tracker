...
export function ItemModal({
  ...
  function onNameChange(val: string) {
    ...
    if (!srdReady || val.length < 2) { setShowDrop(false); return }
  ...
  return (
    ...
    >
      <Field
        label="Name"
        hint={srdReady
          ? <span style={{ color: 'var(--accent)', fontSize: 10 }}>(SRD autocomplete active)</span>
          : undefined}
      >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <Field label="Stored at">
          <select value={location} onChange={e => setLocation(e.target.value)} disabled={!!carrier}>
            <option value="">— none —</option>
      ...
      <div className="field" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <input
          ...
          style={{ flexShrink: 0 }}
        />
        <label htmlFor="item-notable" style={{ fontSize: 13, color: 'var(--text-primary)', textTransform: 'none', letterSpacing: 0 }}>
