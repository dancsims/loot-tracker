...
export function TransactionModal({ transaction, currencies, onSave, onClose }: Props) {
  ...
  function handleSave() {
    ...
    onSave({ id: transaction?.id ?? uid(), date, description: desc.trim(), amounts: parsed, note });
  }
  ...
  return (
    ...
    >
      <Field label="Date">
        <TextField fullWidth size="small" type="date" value={date} onChange={(e) => setDate(e.target.value)} sx={inputSx} inputProps={{ style: { fontSize: 13 } }} />
      </Field>

      <Field label="Description">
        <TextField fullWidth size="small" value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="e.g. Sold goblin loot" sx={inputSx} inputProps={{ style: { fontSize: 13 } }} />
      </Field>
        ...
        <Box sx={{ display: "grid", gap: 0.625 }}>
          {currencies.map((c) => (
            <Grid container key={c.id} alignItems="center" spacing={1}>
              <Grid item xs>
                <Typography fontSize={13}>{c.name} ({c.symbol})</Typography>
              </Grid>
              <Grid item xs={4}>
                <TextField
                  fullWidth
      ...
      <Field label="Note (optional)">
        <TextField fullWidth multiline minRows={2} size="small" value={note} onChange={(e) => setNote(e.target.value)} sx={inputSx} inputProps={{ style: { fontSize: 13 } }} />
      </Field>
    </Modal>
  );
}
