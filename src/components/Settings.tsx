    ...
    <div style={{ minWidth: 700, maxWidth: 900, margin: "0 auto" }}>
      ...
      <div className="settings-section">
        ...
        {state.currencies.map((c) => (
          <div className="list-item" key={c.id}>
            <span>
              <strong style={{ fontWeight: 500 }}>{c.name}</strong> ({c.symbol})
              — 1 {c.symbol} = {c.rate} gp
            </span>
            <Button
              size="sm"
              variant="danger"
      ...
      <div className="settings-section">
        ...
        {state.characters.map((c, i) => (
          <div className="list-item" key={i}>
            <span>{c}</span>
            <Button size="sm" variant="danger" onClick={() => deleteChar(i)}>
              Remove
            </Button>
          </div>
        ))}
      ...
      <div className="settings-section">
        ...
        {state.locations.map((l, i) => (
          <div className="list-item" key={i}>
            <span>{l}</span>
            <Button size="sm" variant="danger" onClick={() => deleteLoc(i)}>
              Remove
            </Button>
          </div>
        ))}
